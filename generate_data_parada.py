...
def datos_paradas_producto():
    url = 'http://192.168.252.6/serviciowebaccesonet/api/authentication/logintercero'
    urlMaquina = f'http://192.168.252.6/serviciowebtercerosnet/api/OrdenProceso/1'
    api = Api(url, urlMaquina)
    data = api.get_all_data()
...
def match_alex_scada_paradas():
    ...
    # Ahora: rellenar idNumOrd y operario por máquina incluso cuando no hubo coincidencia temporal
    df_alex_local = df_alex.copy()
    # ordenar por fecha de inicio Alex si existe para preferir el primero del día
    if 'inicioParada_alex' in df_alex_local.columns:
        df_alex_local['inicioParada_alex'] = pd.to_datetime(df_alex_local['inicioParada_alex'], errors='coerce')
        df_alex_local = df_alex_local.sort_values(by='inicioParada_alex')

    # construir mapa representativo por máquina -> (idNumOrd, operario)
    rep_map = {}
    if 'codMaquina' in df_alex_local.columns:
        for codm, group in df_alex_local.groupby('codMaquina'):
            # elegir el primer idNumOrd y operario no nulos si existen
            id_val = pd.NA
            oper_val = pd.NA
            if 'idNumOrd' in group.columns and not group['idNumOrd'].dropna().empty:
                id_val = group['idNumOrd'].dropna().iloc[0]
            elif 'idNumOrden' in group.columns and not group['idNumOrden'].dropna().empty:
                id_val = group['idNumOrden'].dropna().iloc[0]
            if 'operario' in group.columns and not group['operario'].dropna().empty:
                oper_val = group['operario'].dropna().iloc[0]
            rep_map[codm] = (id_val, oper_val)

    # rellenar en df_copy donde falta idNumOrd u operario, por máquina
    for codm, (id_val, oper_val) in rep_map.items():
        mask_fill = (df_copy['codMaquina'] == codm) & (pd.isna(df_copy['idNumOrd']) | (df_copy['idNumOrd'] == ''))
        if id_val is not pd.NA:
            df_copy.loc[mask_fill, 'idNumOrd'] = id_val
        if oper_val is not pd.NA:
            df_copy.loc[mask_fill, 'operario'] = oper_val
